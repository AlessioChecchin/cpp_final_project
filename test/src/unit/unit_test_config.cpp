...
void unit_test_config::test_category_config()
{
	...
	assert(("Failed to build the category_config:", a.name == "a"));
	assert(("Failed to build the category_config:", a.id == 0));
	assert(("Failed to build the category_config:", a.number == 8));

	prj::config::category_config b{"abcdefg", 2, -203};
	assert(("Failed to build the category_config:", b.name == "abcdefg"));
	assert(("Failed to build the category_config:", b.id == -1));
	assert(("Failed to build the category_config:", b.number == -203));

	prj::config::category_config a{"", 0, 8};
	assert(("Failed to build the category_config:", a.name == ""));
	assert(("Failed to build the category_config:", a.id == 0));
	assert(("Failed to build the category_config:", a.number == 8));
